...
class TestGather(p: PlatformWrapperParams) extends GenericAccelerator(p) {
  ...
  class TestGatherIO() extends GenericAcceleratorIF(numMemPorts, p) {
  ...
  val orderCheckQ = Module(
    new Queue(gather.accel_io.in.bits.cloneType, 2 * numTxns)
  ).io
  ...
  when(!regActive) {
    ...
    when(gather.accel_io.out.ready & gather.accel_io.out.valid) {
      ...
      when(orderCheckQ.deq.bits.tag =/= gather.accel_io.out.bits.tag) {
        printf(
          "Found OoO response at %d, expected %d found %d \n",
          regResultsOK + regResultsNotOK,
          orderCheckQ.deq.bits.tag,
          gather.accel_io.out.bits.tag
        )
