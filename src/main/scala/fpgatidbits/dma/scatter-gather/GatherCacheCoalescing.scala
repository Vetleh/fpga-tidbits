...
class GatherNBCache_Coalescing(
  ...
  class InternalReq extends CloakroomBundle(outstandingTxns) {
    ...
    override val printfElems = { () =>
      Seq(id, cacheLine, cacheTag, cacheOffset)
    }
  }
  ...
  val tagStore = Module(
    new PipelinedDualPortBRAM(
      addrBits = cacheLineNumBits,
      dataBits = 1 + cacheTagBits,
      regIn = 0,
      regOut = pipelinedStorage
    )
  ).io
  ...
  val roc = Module(
    new ReadOrderCacheBRAM(
      new ReadOrderCacheParams(
        mrp = mrp,
        maxBurst = burstBeatsPerLine,
        outstandingReqs = nbMisses,
  ...
  class CoalescingMissHandlerRsp(maxMissPerLine: Int) extends PrintableBundle {
    ...
    override val printfStr =
      "numMisses %d, missed line %d, missed tag %d, cacheline %x \n"
    override val printfElems = { () =>
      Seq(numMisses, misses(0).cacheLine, misses(0).cacheTag, cacheline)
    }
  ...
  class CoalescedMissRspGen(maxMissPerLine: Int) extends Module {
    ...
    val regMisses = RegInit(VecInit(Seq.fill(maxMissPerLine) {
  ...
  class CoalescingMissHandler(maxMissPerLine: Int) extends Module {
    ...
    val pendingLines = Module(
      new CAM(nbMisses, cacheLineNumBits + cacheTagBits)
    ).io

    val regNumMiss = RegInit(
      VecInit(Seq.fill(nbMisses)(0.U((log2Ceil(maxMissPerLine) + 1).W)))
    )
    val regTag = RegInit(
      VecInit(Seq.fill(nbMisses)(0.U((cacheLineNumBits + cacheTagBits).W)))
    )
    ...
    val enterAsNew =
      !pendingLines.hit & pendingLines.hasFree & io.reqOrdered.ready
    val enterAsExisting =
      pendingLines.hit & (regNumMiss(foundLineID) < (maxMissPerLine).U)
