...
class AsymPipelinedDualPortBRAM(
  ...
  if (asymRatio == 1) {
    ...
    val mem = VecInit.fill(asymRatio) {
      Module(
        new PipelinedDualPortBRAM(
          addrBits = addrInUnit_bits,
          dataBits = minWidth,
          regIn = regIn,
          regOut = regOut
        )
      ...
      if (p.readWidth > p.writeWidth) {
        ...
        for (j <- 0 until asymRatio) {
          mem(j)
            .ports(pn)
            .req
            .writeEn := io.ports(pn).req.writeEn & (j.U === addrOfUnit)
        }
        // read data is several unit reads concatenated
        io.ports(pn).rsp.readData := Cat(
          mem.map(_.ports(pn).rsp.readData).reverse
        )
      } else {
        ...
        for (j <- 0 until asymRatio) {
          mem(j).ports(pn).req.writeData := io
            .ports(pn)
            .req
